<?php 
    ...
    if(isset($_POST['submit'])){ 
        ...
        $DateFile = 'galeria/' . $url;

        if(rmdir($DateFile)){
            $Get_ModelS = new Get_Model();
            $get_studies = $Get_ModelS->Get_studios($paciente_file,$fecha,$destino_file);
            
            while($data_studies = mysqli_fetch_assoc($get_studies)){ 
                $id_studies = $data_studies['id'];

                //Elimino las img del Paciente
                $Delete_ModelI = new Delete_Model();
                $delete_gallery = $Delete_ModelI->Delete_gallery($id_studies);
                
                if($delete_gallery){
                    //Elimino el Studio del Paciente
                    $Delete_ModelS = new Delete_Model();
                    $delete_studies = $Delete_ModelS->Delete_studios($id_studies);
                    if($delete_studies){
